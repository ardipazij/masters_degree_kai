...
% Граф ненаправленный, поэтому описываем ребра в обе стороны или 
% обрабатываем это в коде.
...
% Главный предикат: prim(+StartNode, -MST)
prim(StartNode, MST) :-
    graph(Edges),
    % Инициализация: U = {StartNode}, P = {}
    solve_prim([StartNode], Edges, [], MST).

% solve_prim(+VisitedNodes, +AllEdges, +CurrentMST, -FinalMST)
% Условие выхода (Конец): U = V (все вершины посещены).
% В Prolog проще проверять, нельзя ли добавить новое ребро.
...
% Вспомогательная функция для обновления списка посещенных вершин
